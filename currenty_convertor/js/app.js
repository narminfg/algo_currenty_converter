...
let fromInp_currency=document.querySelector(".fromInp_currency")
...
fromInp_currency.addEventListener("keyup",()=>{
   Convert(); 
})
...
function Convert(){
  if(fromValue==toValue){
    fromInp_currency.value!=null?toInp.value=fromInp_currency.value:fromInp_currency.value=toInp.value
    ...
    fetch(`https://api.exchangerate.host/latest?base=${fromValue}&symbols=${toValue}`).then(res=>res.json()).then(data=>{
       console.log("istek getdi")
        ...
        toInp.value= `${(fromInp_currency.value*data.rates[toValue]).toFixed(2)}`
...
function Converto(){
  ...
  if(fromValue==toValue){
    ...
    fetch(`https://api.exchangerate.host/latest?base=${toValue}&symbols=${fromValue}`).then(res=>res.json()).then(data=>{
        ...
        fromInp_currency.value= `${(toInp.value*data.rates[fromValue]).toFixed(2)}`
